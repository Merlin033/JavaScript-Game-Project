...
export class Sitting extends State {
	...
	handleInput(input) {
		if (input.includes("ArrowLeft") || input.includes("ArrowRight") || input.includes("q") || input.includes("d")) {
...
export class Running extends State {
	...
	handleInput(input) {
		...
		} else if (input.includes("ArrowUp") || input.includes("z")) {
...
export class Rolling extends State {
	...
	handleInput(input) {
		...
		if (!input.includes("Enter") && this.game.player.onGround()) {
			...
			this.game.player.setState(states.FALLING, 1);
		} else if (input.includes("Enter") && (input.includes("ArrowUp") || input.includes("z")) && this.game.player.onGround()) {
			this.game.player.vy -= 27;
		} else if ((input.includes("ArrowDown") || input.includes("s")) && !this.game.player.onGround()) {
			this.game.player.setState(states.DIVING, 0);
		}
	}
}
